...
const QuickSortPage = () => {
  ...
  const header: string = "Quick Sort"
  const bestCaseArr: number[] = [1,5,3,12,8,10,15,6]
  const worstCaseArr: number[] = [1,3,5,6,8,10,12,15].reverse()
  ...
  return (
    <div>
      ...
      {selectedTab === 0 && (
        <>
          <p>
            Quick sort is a sorting algorithm based on the divide and conquer approach, that builds the final sorted array/list by picking a pivot element and<br/> 
          ...
          <p>
            The best case scenario for quick sort is the case where the list is randomized and has a running time of O(n log(n)):
          </p>

          <AnimationManager input={{type: "array", data: bestCaseArr}} algorithm={Algorithm.QuickSort} visualizationGraphics={Visualization.ListBar} isInAbout={true}/>
          
          <h4 className="v-space">Worst case performance</h4>
          <p>
            The worst case scenario for quick sort is the case where the list goes from highest to lowest or vice versa and has a running time of O(n<sup>2</sup>):
          </p>

          <AnimationManager input={{type: "array", data: worstCaseArr}} algorithm={Algorithm.QuickSort} visualizationGraphics={Visualization.ListBar} isInAbout={true}/>
          ...
          <br/>
          <a 
            href="https://github.com/PeterWesselLindberg/Algorithm_Analysis_Tool/blob/main/src/algorithms/quickSort.ts" target="_blank">
